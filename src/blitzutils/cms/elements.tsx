...
const Actions: React.FC<ActionsProps> = ({ row, name }) => {
    const router = useRouter()

    return <>
        <button onClick={e => router.push(`${name.replaceAll(' ', '-').toLowerCase()}/update/${row.id}`)} className="bg-yellow-100 border-yellow-300 px-4 py-2 border-2 rounded-sm font-medium mr-2 w-fit">
        ...
        <button onClick={e => router.push(`${name.replaceAll(' ', '-').toLowerCase()}/delete/${row.id}`)} className="bg-red-200 border-red-500 px-4 py-2 border-2 rounded-sm font-medium w-fit">
            <svg xmlns="http://www.w3.org/2000/svg" fill="#000000" version="1.1" id="Capa_1" width="20px" height="20px" viewBox="0 0 482.428 482.429">
                <g>
                    <g>
...
const RowImage = ({ src }: { src: string }) => {
    return <div className="h-[7.45vh]"><Image layout="fill" objectFit="contain" src={src} alt="Row Image" /></div>
}

export { Actions, RowImage }